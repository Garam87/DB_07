...
-- > ANY : 과장급여 3개중, 최고금액이 220 보다 더 큰 SALARY
...
SELECT EMP_NAME , JOB_CODE , SALARY,
(SELECT CEIL(AVG(SALARY)) FROM EMPLOYEE SUB WHERE SUB.JOB_CODE = MAIN.JOB_CODE)
FROM EMPLOYEE MAIN
...
NVL((SELECT EMP_NAME FROM EMPLOYEE SUB WHERE MAIN.MANAGER_ID = SUB.EMP_ID), '없음') 관리자명
FROM EMPLOYEE MAIN
